...
from graphics import RESOLUTION, TTY_FONT_SIZE, BETTER_WHITE, WHITE, BLACK, UM_FNT_PT_FACTOR
...
class TeleTypeWriter(Application):
    ...
    def __init__(self, host: OperatingSystem, opened_by: OperatingSystem | None = None) -> None:
        ...
        self.surface = self.host.graphics.get_surface(*RESOLUTION, [0, 0])
        ...
        self.text: Text = Text(self.content + self.stdin, WHITE, 'regular', self.font_size, (5, 5), self.surface.get_width(), self.surface.get_height(), (5, 5))
    ...
    def graphics_handler(self) -> None:
        ...
        for section in self.processed_text:
            if self.surface.get_height() - 5 > section.pos[1] >= 5:
    ...
    def events_handler(self) -> None:
        ...
        for event in self.events:
            ...
            if event.type in [pygame.TEXTINPUT, pygame.KEYDOWN]:
                ...
                if self.get_number_of_lines() > self.max_lines:
                    self.text.update_starting_pos((5, 5 - (self.text.font_size * UM_FNT_PT_FACTOR[1] * (self.bottom_line - self.max_lines))))
                else:
                    self.text.update_starting_pos((5, 5))
